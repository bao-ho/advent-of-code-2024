...
const findPathToNine = ({ height, x, y, path }) => {
  const nextHeight = height + 1;
  //north
  if (y - 1 >= 0 && map[x][y - 1] === nextHeight) {
    if (nextHeight === 9)
      distinctPaths.add(`${path}${JSON.stringify({ x, y: y - 1 })}`);
    else
      findPathToNine({
        height: nextHeight,
        x,
        y: y - 1,
        path: `${path}${JSON.stringify({ x, y: y - 1 })}`,
      });
  }
  //east
  if (x + 1 < WIDTH && map[x + 1][y] === nextHeight) {
    if (nextHeight === 9)
      distinctPaths.add(`${path}${JSON.stringify({ x: x + 1, y })}`);
    else
      findPathToNine({
        height: nextHeight,
        x: x + 1,
        y,
        path: `${path}${JSON.stringify({ x: x + 1, y })}`,
      });
  }
  //south
...
for (let y = 0; y < HEIGHT; y++) {
  for (let x = 0; x < WIDTH; x++) {
    ...
    findPathToNine({ height: 0, x, y, path: JSON.stringify({ x, y }) });
    sum += distinctPaths.size;
  }
}
